...
export const generatePdfFile = (type: string, name: string, job: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    doc.pipe(stream);
    doc.fontSize(25).text(`Informations sur la profession:${name} est ${job} dans le ${type}  `, 150, 100);
    doc.end();
